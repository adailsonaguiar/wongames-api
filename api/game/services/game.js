...
async function getGameInfo(slug) {
  ...
  const raitingElement = document.querySelector(".age-restrictions__icon use");
  const description = document.querySelector(".description");

  return {
    raiting: raitingElement
      ? raitingElement
          .getAttribute("xlink:href")
          .replace(/_/g, "")
          .replace(/[^\w-]+/g, "")
      : "Free",
    short_description: description.textContent.trim().slice(0, 50) + "...",
  ...
  await Promise.all(
    products.map(async (product) => {
      ...
      if (!item) {
        ...
        try {
          const infos = await getGameInfo(product.slug);
          infos.short_description = String(infos.short_description);
